...
class Store:
    """
    check_same_thread=False lets different threads USE this connection, but a
    sqlite3.Connection is still not safe for truly concurrent statements —
    FastAPI runs each sync endpoint in its own thread-pool thread, so two
    manual-control requests arriving together could hit the connection at the
    same instant (confirmed live: a 4-way Promise.all() from the dashboard
    threw "bad parameter or other API misuse"). A single lock around every
    method serializes access without needing a connection pool.
    """
